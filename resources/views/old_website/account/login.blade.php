...
@section('content')
    ...
    <form id="signUp" method="post" action="{{ route('signInSite') }}" enctype="multipart/form-data" class="form-horizontal" role="form">
    {{ csrf_field() }}
        ...
        <div class="bottom-form-text">
            <h5> {{ __('translate.dontHaveAccount') }} <a href="{{ route('signUpSiteForm') }}"> {{ __('translate.subscribeNow') }} </a></h5>
        </div>
    </form>
